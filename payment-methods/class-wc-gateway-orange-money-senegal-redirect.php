...
class WC_Akouendy_OrangeMoneySnRedirect extends WC_Payment_Gateway {

    public function __construct() {
        $this->akouendy_errors = new WP_Error();
        $this->partner = 'default';
        $this->id = 'akd-orange-money-sn';
        $this->method_title = '[Akouendy]Orange Money Sénégal';
            ...
            try {
                ...
                if ($respond_code == "00") {
                    ...
                    if ($wc_order_id !== $trxId[0]) {
                        $message = "Votre session de transaction a expir&eacute;. ";
                        $message_type = "notice";
                    ...
                    if ($status == "SUCCESS") {
                        ...
                        $message = "Merci pour votre achat. Le paiement a &eacute;t&eacute; re&ccedil;u. Votre commande est en cours de traitement.";
                        $message_type = "success";
                        $order->payment_complete();
                        $order->update_status('completed');
                        $order->add_order_note('Paiement AKOUENDY effectu&eacute; avec succ&egrave;s<br/>ID: ' . $token);
                        ...
                        $message = "Le paiement n'a pu &egrave;tre compl&eacute;t&eacute;e.";
                        $message_type = "error";
                        $order->add_order_note("Le paiement a &eacute;chou&eacute; ou l'utilisateur a eu &agrave; une faire demande d'annulation de paiement");
                        $redirect_url = $order->get_cancel_order_url();
                    }
                } else {
                    //payment not found
                ...
                if (version_compare(WOOCOMMERCE_VERSION, "2.2") >= 0) {
                    $order->update_meta_data('_akouendy_hash', $hash);
                }
